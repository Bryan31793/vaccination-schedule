...
if __name__ == "__main__":
    # Ejecutar simulación
    hist_pos, hist_estado, hist_aristas, hist_sir, n_frames = run_simulation(params)
    
    # Estadísticas
    I_vals = [x[1] for x in hist_sir]
    ...
    sir_comp = setup_sir_panel(components['ax_sir'], params, colors, hist_sir)
    add_title(fig, params)
    
    # Crear función de actualización
    update_func = create_update_function(
        hist_pos, hist_estado, hist_aristas, hist_sir,
        world_comp, sir_comp, colors
    )
    
    # Exportar animación
    export_animation(fig, update_func, n_frames, params, colors)