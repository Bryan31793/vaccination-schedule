...
const BIENVENIDA: ChatMessage = {
  id: '0',
  text: '¡Hola! Soy tu asistente de salud personal. Puedo resolver dudas sobre vacunas, efectos secundarios, recomendaciones y tu esquema de vacunación. ¿En qué te puedo ayudar?',
  isUser: false,
};

export const AsistenteCiudadanoScreen: React.FC = () => {
  const { token } = useAuth();
  ...
  const mutation = useMutation({
    mutationFn: (mensaje: string) => ciudadanoApi.chatbot(token!, mensaje),
    onSuccess: (data) => {
      setMessages(prev => [
        ...prev,
    ...
    onError: () => {
      setMessages(prev => [
        ...prev,
        {
          ...
          text: 'No pude conectarme con el asistente. Intenta de nuevo.',
          isUser: false,
        },
      ]);
    },
    ...
    <View style={[styles.bubble, item.isUser ? styles.userBubble : styles.aiBubble]}>
      {!item.isUser && (
        <View style={styles.aiHeader}>
          <LinearGradient
            colors={[colors.secondary[500], colors.secondary[300]] as [string, string]}
            style={styles.aiAvatar}
          >
            <Ionicons name="sparkles" size={13} color="#FFF" />
          </LinearGradient>
          <Text style={styles.aiLabel}>Asistente de Salud</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  aiLabel:   { ...typography.caption, fontWeight: '600', color: colors.secondary[500] },
  ...
  typingText: { ...typography.bodySmall, color: colors.secondary[500], fontStyle: 'italic' },
  inputBar: {
    flexDirection: 'row', alignItems: 'flex-end',
    paddingHorizontal: 12, paddingVertical: 10, paddingBottom: 12,
    backgroundColor: colors.neutral[0],
  ...
  input: {
    ...
    fontSize: 15, color: colors.neutral[800],
